...
IGMO_TRACING_ENABLED="${IGMO_TRACING_ENABLED:-false}"
IGMO_TRACING_SAMPLING_PROBABILITY="${IGMO_TRACING_SAMPLING_PROBABILITY:-0.1}"
IGMO_TRACING_OTLP_ENDPOINT="${IGMO_TRACING_OTLP_ENDPOINT:-https://otlp-gateway-prod-ap-northeast-0.grafana.net/otlp/v1/traces}"
IGMO_TRACING_OTLP_AUTHORIZATION="${IGMO_TRACING_OTLP_AUTHORIZATION:-}"
GRAFANA_CLOUD_INGEST_TOKEN="${GRAFANA_CLOUD_INGEST_TOKEN:-}"
GRAFANA_CLOUD_TRACES_USERNAME="${GRAFANA_CLOUD_TRACES_USERNAME:-1703597}"
OTEL_SERVICE_NAME="${OTEL_SERVICE_NAME:-igmo}"
...
case "$IGMO_TRACING_ENABLED" in
  true|false) ;;
  *)
    echo 'IGMO_TRACING_ENABLED must be true or false.' >&2
    exit 1
    ;;
esac

if [[ ! "$IGMO_TRACING_SAMPLING_PROBABILITY" =~ ^((0(\.[0-9]+)?)|(1(\.0+)?))$ ]]; then
  echo 'IGMO_TRACING_SAMPLING_PROBABILITY must be between 0.0 and 1.0.' >&2
  exit 1
fi

if [[ "$IGMO_TRACING_ENABLED" == true ]]; then
  if [[ -z "$IGMO_TRACING_OTLP_AUTHORIZATION" ]]; then
    require_value GRAFANA_CLOUD_INGEST_TOKEN
    IGMO_TRACING_OTLP_AUTHORIZATION="Basic $(printf '%s' "$GRAFANA_CLOUD_TRACES_USERNAME:$GRAFANA_CLOUD_INGEST_TOKEN" | base64 | tr -d '\n')"
  fi
fi
...
runtime_environment() {
  ...
  dotenv_line IGMO_TRACING_ENABLED "$IGMO_TRACING_ENABLED"
  dotenv_line IGMO_TRACING_SAMPLING_PROBABILITY "$IGMO_TRACING_SAMPLING_PROBABILITY"
  dotenv_line IGMO_TRACING_OTLP_ENDPOINT "$IGMO_TRACING_OTLP_ENDPOINT"
  dotenv_line IGMO_TRACING_OTLP_AUTHORIZATION "$IGMO_TRACING_OTLP_AUTHORIZATION"
  dotenv_line OTEL_SERVICE_NAME "$OTEL_SERVICE_NAME"
