...
class GrafanaDashboardSyncTest(unittest.TestCase):
    ...
    def write_fake_curl(self, mode):
        ...
        script.write_text(
            ...
            "      dashboard-mismatch) printf '%s' '{\"uid\":\"dashboard-one\",\"title\":\"Wrong\",\"folderUid\":\"igmo-folder\",\"version\":1}' > \"$output\" ;;\n"
            "      valid|partial|same-title-different-uid)\n"
            "        dashboard_uid=${url##*/}\n"
            "        title=\"Dashboard One\"\n"
            "        if [ \"$dashboard_uid\" = dashboard-two ]; then title=\"Dashboard Two\"; fi\n"
            "        printf '{\"uid\":\"%s\",\"title\":\"%s\",\"folderUid\":\"igmo-folder\",\"version\":2}' \"$dashboard_uid\" \"$title\" > \"$output\" ;;\n"
